...
class YOLO(object):
    _defaults = {
        ...
        # --------------------------------------------------------------------------#
        "model_path": 'D:/runcode/Yolov5-Deepsort-Fastreid-main/two_step/model_data/best_epoch_weights.pth', # "model_path": 'D:/runcode/Yolov5-Deepsort-Fastreid-main/two_step/model_data/best_epoch_weights.pth'
    ...
    def detect_image(self, image):
        ...
        return np.array(bbox_xywh), cls_conf, cls_ids, xy  # np.array(bbox_xywh)

    def matching_body(self, face_xywh, body_boxes, ori_fac):
        save_bodypic = []
        matched_pairs = []
        output_folder = 'D:/runcode/Yolov5-Deepsort-Fastreid-main/fast_reid/query'
        found_person_folder = os.path.join(output_folder, 'a1111111')
        os.makedirs(found_person_folder, exist_ok=True)
        overlap_threshold = 0.6
        for face in face_xywh:
            for body_box in body_boxes:
                overlap = overlap_ratio(face, body_box)
                if overlap >= overlap_threshold:
                    matched_pairs.append(body_box)
                    for i, b in enumerate(matched_pairs):
                        b = list(map(int, b))
                        # if name != "Unknown":
                        x1 = max(0, b[0])
                        y1 = max(0, b[1])
                        x2 = min(ori_fac.shape[1], b[2])
                        y2 = min(ori_fac.shape[0], b[3])
                        if x2 > x1 and y2 > y1:
                            crop_img = ori_fac[y1:y2, x1:x2]
                            save_bodypic.append(crop_img)
        for idx, f in enumerate(save_bodypic):
            # RGBtoBGR满足opencv显示格式
            f = cv2.cvtColor(f, cv2.COLOR_RGB2BGR)
            filename = f'a1-{idx + 1}.jpg'
            # Construct the full path to save the image
            output_path = os.path.join(found_person_folder, filename)
