...
from util.utils import (Alignment_1, compare_faces, letterbox_image,
                        preprocess_input)
from util.utils_bbox_rtsp import (decode, decode_landm, non_max_suppression,
...
class Retinaface(object):
    _defaults = {
        ...
        #   retinaface训练完的权值路径
        # ----------------------------------------------------------------------#
        "retinaface_model_path": 'model/Retinaface_resnet50.pth',
        # ----------------------------------------------------------------------#
        ...
        #   训练完的权值路径
        # ----------------------------------------------------------------------#
        "Re_model_path": 'model/backbone_ir50_asia.pth',
        # ----------------------------------------------------------------------#
        #   使用的主干网络
        # ----------------------------------------------------------------------#
        "backbone": "Ir_50",
    ...
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image_tensor, txt_path, t1, cam_addrs):
        # ---------------------------------------------------#
        #   对输入图像进行一个备份，后面用于绘图
        # ---------------------------------------------------#
        for url, image_tensor in image_tensor.items():
            old_image = image_tensor.clone().detach().cpu().numpy()
            old_image = old_image.transpose(1, 2, 0)
            old_image = old_image.astype(np.uint8)
            image = image_tensor.float().unsqueeze(0)
