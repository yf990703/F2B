...
def letterbox_image(image, size):
    # get the height and width of the input image tensor
    ih, iw = image.shape[-2:]
    w, h = size
    ...
    image = torch.nn.functional.interpolate(image, size=(nh, nw), mode='bilinear', align_corners=False)

    new_image = torch.full((3, size[1], size[0]), 128, dtype=image.dtype, device=image.device)
    new_image[:, (h - nh) // 2:(nh + (h - nh) // 2), (w - nw) // 2:(nw + (w - nw) // 2)] = image
...
#   比较人脸
# ---------------------------------#
def compare_faces(known_face_encodings, face_encoding_to_check, tolerance=50):
